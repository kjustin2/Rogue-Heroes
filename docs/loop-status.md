...
| 19 | Per-map prop variety (≥6 theme kinds per map) — coordinator gap for asks 2/7 | art agent | RUNNING | one GPU shot per map |
| 20 | Hit reactions for cannon/mortar/flamer/bomb — coordinator gap for ask 1 | projectile agent | RUNNING | filmstrip frame per family |
